...
public class Paging {
	...
	public String makeHtmlPaging() {
		...
		int totalPage = (maxNum % listCount > 0)
				?maxNum/listCount+1 : maxNum/listCount;//글의 수가 32개이면 페이지 수가 4개 필요하다. 나머지 2개의 글
		// 전체 페이지 그룹 갯수
	...
	private String makeHtml(int currentGroup, int totalPage, String boardName) {
		...
		if (end != totalPage) { //마지막 그룹에 하나의 페이지만 있으면 end와 totalPage는 같으므로 실행 안됨
